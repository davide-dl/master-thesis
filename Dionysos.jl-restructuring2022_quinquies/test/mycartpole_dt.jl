...
min_tstep = 0.04
max_tstep = 0.04
# sys1 = ST.NewSimpleSystemRK4(tstep,f,nsys,SVector(0.0,0.0,0.0,0.0),SVector(0.0))
RKsys = ST.NewVarTSystemRK4(f,nsys,SVector(0.0,0.0,0.0,0.0),SVector(0.0),min_tstep,max_tstep)
sys1 = ST.NewVarTDiscreteSystem(f,F,SVector(0.0,0.0,0.0,0.0),SVector(0.0),min_tstep,max_tstep)
...
fig = Plots.plot(aspect_ratio = x_step/theta_step,legend = false)
...
@time SR.buildAll!(sys_rel,simulate_borders=false,look_at_corners=false,var_tstep=1)
...
@time C.plot_costs_at_origin!(sys_rel,rewards,target=T,fig=fig)

xi1 = SVector(-2.0,-0.0,0.0,0.0)
xi2 = SVector(-3.0,-10.0,0.0,0.0)
xi3 = SVector(-4.0,-10.0,0.0,0.0)
...
C.simulateRK4(RKsys,sys_rel,xi1,T,best_actions,rewards,fig,tstep=0,render_each_step=true)
C.simulateRK4(RKsys,sys_rel,xi2,T,best_actions,rewards,fig,tstep=0,render_each_step=true)
C.simulateRK4(RKsys,sys_rel,xi3,T,best_actions,rewards,fig,tstep=0,render_each_step=true)
# C.simulate(sys_rel,xi,T,best_actions,costs,fig,tstep=0.02,render_each_step=true)
